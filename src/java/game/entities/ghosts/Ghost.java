...
public abstract class Ghost extends MovingEntity {
    ...
    public void update() {
        ...
        //상태에 따라 5초 또는 20초가 지나면 적절한 전환을 적용하기 위해 해당 상태(state)에 알림을 보냅니다.
        if (state == chaseMode || state == scatterMode) {
            modeTimer++;

            //(추격)일 땐 20초, (흩어지기)일 땐 5초간 지속
    ...
    public void render(Graphics2D g) {
        ...
        if (state == frightenedMode) { //겁먹은' 상태일 때 파란색 유령(frightenedSprite1)을 그립니다.
            //7초 중 마지막 2초가 되면 파란색/하얀색(frightenedSprite2)을 번갈아 그리며 "곧 풀린다"는 깜빡임 효과를 줍니다.
